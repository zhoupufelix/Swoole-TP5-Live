...
class Ws{
    ...
    public function onWorkerStart($server,$worker_id){
        ...
        $table->column('phone_num', \swoole_table::TYPE_STRING, 64);
        $table->column('last_update_time', \swoole_table::TYPE_INT, 4);       //1,2,4,8
        $table->column('token', \swoole_table::TYPE_STRING, 64);
        $table->create();
        $server->user = $table;
    }
...
//        return "onTask finish";//告诉worker
    ...
    public function onRequest($request,$response)
    {
        ...
        if ( !empty($_POST['cookie']) ){
            foreach($_POST['cookie'] as $k=>$cookie){
                $expire = isset($cookie['value']['expire'] )?$cookie['value']['expire']:0;
                $path   = isset($cookie['value']['path'] )?$cookie['value']['path']:'/';
                $response->cookie($k, $cookie['value'] ,$expire , $path);
            }
        }
